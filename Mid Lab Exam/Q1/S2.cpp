...
int main()
{
    ...
    // Sending S2's port number to C2
    string PORT_str = to_string(PORT);
    char* msg = (char*)PORT_str.c_str();

    send(c2fd, msg, strlen(msg), 0);


    // Establishing a connectionless thingy
    ...
    sockaddr_in s2_addr, c2_addr;

    s2_addr.sin_family = AF_INET;
    s2_addr.sin_addr.s_addr = INADDR_ANY;
    s2_addr.sin_port = htons(PORT);

    if(bind(sfd, (struct sockaddr*) &s2_addr, sizeof(s2_addr)) < 0)
    {
        perror("bind FAILED");
        exit(1);
    }

    int len = sizeof(c2_addr);

    while(1)
    {
        char buffer[BUFF_LEN];
        memset(buffer, 0, sizeof(buffer));
        while(!recvfrom(sfd, buffer, sizeof(buffer), MSG_WAITALL, (struct sockaddr *) &c2_addr, (socklen_t *) &len));
